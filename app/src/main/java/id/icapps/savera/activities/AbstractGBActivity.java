            ...
            if (GBApplication.isDarkThemeEnabled()) {
                if ((flags & NO_ACTIONBAR) != 0) {
                    if (GBApplication.isAmoledBlackEnabled())
                        activity.setTheme(R.style.SaveraUiThemeDynamicDarkAmoled_NoActionBar);
                    else
                        activity.setTheme(R.style.SaveraUiThemeDynamicDark_NoActionBar);
                } else {
                    if (GBApplication.isAmoledBlackEnabled())
                        activity.setTheme(R.style.SaveraUiThemeDynamicDarkAmoled);
                    else
                        activity.setTheme(R.style.SaveraUiThemeDynamicDark);
                }
            } else {
                if ((flags & NO_ACTIONBAR) != 0) {
                    activity.setTheme(R.style.SaveraUiThemeDynamicLight_NoActionBar);
                } else {
                    activity.setTheme(R.style.SaveraUiThemeDynamicLight);
                }
            }
        } else if (GBApplication.isDarkThemeEnabled()) {
            if ((flags & NO_ACTIONBAR) != 0) {
                if (GBApplication.isAmoledBlackEnabled())
                    activity.setTheme(R.style.SaveraUiThemeBlack_NoActionBar);
                else
                    activity.setTheme(R.style.SaveraUiThemeDark_NoActionBar);
            } else {
                if (GBApplication.isAmoledBlackEnabled())
                    activity.setTheme(R.style.SaveraUiThemeBlack);
                else
                    activity.setTheme(R.style.SaveraUiThemeDark);
            }
        } else {
            if ((flags & NO_ACTIONBAR) != 0) {
                activity.setTheme(R.style.SaveraUiTheme_NoActionBar);
            } else {
                activity.setTheme(R.style.SaveraUiTheme);
            }
        }
