...
package id.icapps.savera.service.devices.cmfwatchpro;
...
public class CmfWatchProSupport extends AbstractBTLEDeviceSupport implements CmfCharacteristic.Handler {
    ...
    public void onNotification(final NotificationSpec notificationSpec) {
        ...
        final String senderOrTitle = id.icapps.savera.util.StringUtils.getFirstOf(
                notificationSpec.sender,
                notificationSpec.title
        );

        final String body = id.icapps.savera.util.StringUtils.getFirstOf(notificationSpec.body, "");

        final byte[] senderOrTitleBytes = id.icapps.savera.util.StringUtils.truncateToBytes(senderOrTitle, 20); // TODO confirm max
        final byte[] bodyBytes = id.icapps.savera.util.StringUtils.truncateToBytes(body, 128); // TODO confirm max
    ...
    public void onSetContacts(final ArrayList<? extends Contact> contacts) {
        ...
        for (final Contact contact : contacts) {
            final byte[] nameBytes = id.icapps.savera.util.StringUtils.truncateToBytes(contact.getName(), 32);
            buf.put(nameBytes);
            buf.put(new byte[32 - nameBytes.length]);

            final byte[] numberBytes = id.icapps.savera.util.StringUtils.truncateToBytes(contact.getNumber(), 25);
            buf.put(numberBytes);
    ...
    public void onSetAlarms(final ArrayList<? extends Alarm> alarms) {
        ...
        for (final Alarm alarm : alarms) {
            ...
            final byte[] labelBytes = id.icapps.savera.util.StringUtils.truncateToBytes(alarm.getTitle(), 8);
    ...
    private void sendMusicStateToDevice() {
        ...
        if (musicSpec != null) {
            track = id.icapps.savera.util.StringUtils.truncateToBytes(musicSpec.track, 63);
            artist = id.icapps.savera.util.StringUtils.truncateToBytes(musicSpec.artist, 63);
        } else {
            track = new byte[0];
            artist = new byte[0];
        }
    ...
    public void onSendWeather(final ArrayList<WeatherSpec> weatherSpecs) {
        ...
        final byte[] locationNameBytes = id.icapps.savera.util.StringUtils.truncateToBytes(weatherSpec.location, 30);
