...
public class ManualSamplesParser extends XiaomiActivityParser {
    ...
    public boolean parse(final XiaomiSupport support, final XiaomiActivityFileId fileId, final byte[] bytes) {
        if (fileId.getVersion() < 2) {
            LOG.warn("Unknown manual samples version {}, skipping", fileId.getVersion());
            return false;
        }
        if (fileId.getVersion() > 2) {
            LOG.warn("Unknown manual samples version {}, attempting to parse as v2", fileId.getVersion());
        }
