...
package id.icapps.savera.service.devices.casio;
...
import id.icapps.savera.util.BcdUtil;
...
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_TIMEFORMAT_AUTO;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_TIMEFORMAT_12H;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_TIMEFORMAT_24H;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_DATEFORMAT;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_DATEFORMAT_AUTO;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_DATEFORMAT_DAY_MONTH;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_DATEFORMAT_MONTH_DAY;
...
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_LIGHT_DURATION_LONGER;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_POWER_SAVING;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_CONNECTION_DURATION;
import static id.icapps.savera.activities.devicesettings.DeviceSettingsPreferenceConst.PREF_TIME_SYNC;
...
public abstract class Casio2C2DSupport extends CasioSupport {
    ...
    public class DeviceAlarms extends DeviceItems<Alarm> {
        ...
        public final boolean mergeValues(byte[][] data, byte[][] previous, SharedPreferences.Editor editor) {
            ...
            for (id.icapps.savera.entities.Alarm alarm : DBHelper.getAlarms(gbDevice)) {
            ...
            for (int i = 0; i < maxAlarms; i++) {
                if (!foundAlarms.contains(i)) {
                    id.icapps.savera.entities.Alarm alarm = AlarmUtils.createDefaultAlarm(gbDevice, i);
                    if (alarm == null) {
                        continue;
                    }
        ...
        public boolean readValues(byte[][] data, id.icapps.savera.entities.Alarm alarm, boolean forceStore) {
            int pos = alarm.getPosition();
            int alm = pos == 0 ? 0 : 1;
            int index = pos == 0 ? 1 : 1 + 4 * (pos - 1);
            if (data.length <= alm || index + 4 > data[alm].length) {
        ...
        public boolean readValues(byte[] data, int index, id.icapps.savera.entities.Alarm alarm, boolean forceStore) {
