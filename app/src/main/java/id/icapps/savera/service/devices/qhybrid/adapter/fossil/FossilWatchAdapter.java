...
package id.icapps.savera.service.devices.qhybrid.adapter.fossil;

import static id.icapps.savera.service.devices.qhybrid.QHybridSupport.ITEM_STEP_GOAL;
import static id.icapps.savera.service.devices.qhybrid.QHybridSupport.ITEM_TIMEZONE_OFFSET;
import static id.icapps.savera.service.devices.qhybrid.QHybridSupport.ITEM_VIBRATION_STRENGTH;
import static id.icapps.savera.service.devices.qhybrid.QHybridSupport.QHYBRID_EVENT_BUTTON_PRESS;
import static id.icapps.savera.service.devices.qhybrid.QHybridSupport.QHYBRID_EVENT_MULTI_BUTTON_PRESS;
...
import id.icapps.savera.devices.qhybrid.NotificationConfiguration;
import id.icapps.savera.devices.qhybrid.PackageConfigHelper;
...
import id.icapps.savera.service.devices.qhybrid.adapter.WatchAdapter;
import id.icapps.savera.service.devices.qhybrid.buttonconfig.ConfigFileBuilder;
import id.icapps.savera.service.devices.qhybrid.buttonconfig.ConfigPayload;
...
import id.icapps.savera.service.devices.qhybrid.requests.Request;
...
import id.icapps.savera.service.devices.qhybrid.requests.fossil.alarm.AlarmsSetRequest;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.configuration.ConfigurationPutRequest;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.device_info.DeviceInfo;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.device_info.DeviceSecurityVersionInfo;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.device_info.GetDeviceInfoRequest;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.device_info.SupportedFileVersionsInfo;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.file.FileDeleteRequest;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.file.FileLookupAndGetRequest;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.file.FilePutRequest;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.file.FirmwareFilePutRequest;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.notification.NotificationFilterPutRequest;
import id.icapps.savera.service.devices.qhybrid.requests.fossil.notification.PlayTextNotificationRequest;
import id.icapps.savera.service.devices.qhybrid.requests.misfit.AnimationRequest;
import id.icapps.savera.service.devices.qhybrid.requests.misfit.MoveHandsRequest;
import id.icapps.savera.service.devices.qhybrid.requests.misfit.ReleaseHandsControlRequest;
import id.icapps.savera.service.devices.qhybrid.requests.misfit.RequestHandControlRequest;
import id.icapps.savera.service.devices.qhybrid.requests.misfit.SaveCalibrationRequest;
...
public class FossilWatchAdapter extends WatchAdapter {
    ...
    public void vibrate(id.icapps.savera.service.devices.qhybrid.requests.misfit.PlayNotificationRequest.VibrationType vibration) {
        // queueWrite(new id.icapps.savera.service.devices.qhybrid.requests.misfit.PlayNotificationRequest(vibration, -1, -1));
    }
    ...
    public void onSetAlarms(ArrayList<? extends Alarm> alarms) {
        ...
        if (alarms.toArray().length == 1 && alarms.get(0).getRepetition() == 0) { //single shot?
            ...
            alarms = (ArrayList<? extends Alarm>) AlarmUtils.mergeOneshotToDeviceAlarms(getDeviceSupport().getDevice(), (id.icapps.savera.entities.Alarm) oneshot, 5);
        }

        ArrayList<id.icapps.savera.service.devices.qhybrid.requests.fossil.alarm.Alarm> activeAlarms = new ArrayList<>();
        for (Alarm alarm : alarms) {
            if (!alarm.getEnabled() || alarm.getUnused()) continue;
            if (alarm.getRepetition() == 0) {
                activeAlarms.add(new id.icapps.savera.service.devices.qhybrid.requests.fossil.alarm.Alarm(
        ...
        queueWrite(new AlarmsSetRequest(activeAlarms.toArray(new id.icapps.savera.service.devices.qhybrid.requests.fossil.alarm.Alarm[0]), this) {
            @Override
            public void onFilePut(boolean success) {
                super.onFilePut(success);
                if (success)
                    GB.toast(getContext().getString(R.string.user_feedback_miband_set_alarms_ok), Toast.LENGTH_SHORT, GB.INFO);
                else
                    GB.toast(getContext().getString(R.string.user_feedback_miband_set_alarms_failed), Toast.LENGTH_SHORT, GB.INFO);
            }
        });
    }
