...
import id.icapps.savera.devices.banglejs.BangleJSActivityPoint;
import id.icapps.savera.devices.banglejs.BangleJSWorkoutParser;
...
class BangleJSActivityTrack {
    ...
    private static void parseFetchedRecorderCSV(File dir, String filename, String log, GBDevice device, Context context) {
        ...
        try {
            ...
            String fileName = FileUtils.makeValidFileName("saverawatch-" + trackType.toLowerCase() + "-" + summary.getName() + ".gpx");
            dir = new File(FileUtils.getExternalFilesDir() + "/" + FileUtils.makeValidFileName(device.getName()));
