...
package id.icapps.savera.service.devices.xiaomi.activity;
...
public class XiaomiActivityFileFetcher {
    ...
    private XiaomiActivityFileId mCurrentRequestedFileId;
    ...
    public void addChunk(final byte[] chunk) {
        ...
        if (num == total) {
            ...
            if (mCurrentRequestedFileId != null && !mCurrentRequestedFileId.equals(fileId)) {
                LOG.warn("Received different activity file than requested. requested={} received={}", mCurrentRequestedFileId, fileId);
            }
            ...
            try {
                parsedOk = activityParser.parse(mHealthService.getSupport(), fileId, data);
                if (parsedOk) {
            ...
            // Only ACK after successful parse so data is not lost on the band if parsing fails
            if (parsedOk && !XiaomiPreferences.keepActivityDataOnDevice(mHealthService.getSupport().getDevice())) {
    ...
    private void triggerNextFetch() {
        ...
        mCurrentRequestedFileId = fileId;
